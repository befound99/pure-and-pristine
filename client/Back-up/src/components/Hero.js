import React from "react";
import { motion } from "framer-motion";

const Hero = () => {
  return (
    <motion.div
      className=" heroImage flex justify-center items-center  text-white h-[calc(100vh-80px)] mt-[80px] box-shadow bg-cover bg-center  snap-start snap-always "
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 0.25 }}
    >
      <div className="flex flex-col items-center w-fit px-1 lg:px-0 lg:pl-8">
        {/* Hero title */}
        <h1 className="monserrat text-[clamp(1.75rem,5vw,4rem)] text-center ">
          Experience the Ultimate Cleanliness
        </h1>
        {/* Hero subtitle */}
        <p className="monserrat text-base text-center  sm:text-2xl lg:text-3xl w-full">
          Unleash the Refreshing Power of Clean
        </p>
        {/* CTA button */}

        <motion.a
          className=" flex justify-center items-center backdrop-brightness-75 border-2 font-normal px-[10px] py-[5px]  md:px-[20px] md:py-[10px] rounded-lg  border10 text10 mt-8 w-fit"
          href="#services"
          initial={{ x: "-100vw" }}
          animate={{ x: 1 }}
          whileHover={{
            boxShadow: "0 0 25px rgb(5, 170, 179)",
            backgroundColor: "#05aab3",
            color: "whitesmoke",
          }}
          transition={{
            duration: 0.5,
            delay: 0.25,
            type: "spring",
            stiffness: 100,
          }}
        >
          <span className=" text-xs sm:text-sm md:text-base lg:text-xl font-medium">
            Elevate Your Space with Pure & Pristine
          </span>
          <i className="fa-solid fa-arrow-up-right-from-square ml-2 text-[12px] sm:text-sm md:text-base lg:text-xl"></i>
        </motion.a>
      </div>
    </motion.div>
  );
};

export default Hero;
